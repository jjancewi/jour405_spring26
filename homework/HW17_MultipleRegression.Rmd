...
Some the variables with the highest correlation are pct_bachelors_higher and turnout (high positive correlation), and pct_white also has positive correlation with turnout. The one that looks to have no correlation with turnout is population. Large and small countries can vary. Based on the scatter plots, the variables that seem to have a linear relationship with the dependent variable are pct_white, income, and median age. Those have a positive linear relationship while unemployment has a slight downward trend. Population has no linear relationship with the dependent variable.
...
While education and unemployment showed strong correlations with turnout in the correlation matrix, once you can control for race and age in the same model their no longer statistically significant. This means those variables are overlapping and diluting the significance of the data, and should not be used together in the same model. Overall, this model is still pretty useful based on what I have seen. It explains the variation in the voter turnout, it identifies 3 meaningful predictors (age, race, income), and it shows that some the variables like population and education may not matter as much as we previously thought.
...
Models 2 and 3 are significantly better than model 4. This is because in model 4 we removed pct_bachelors_higher and pct_white. This shows that those two variables have the highest correlation with turnout and are best for improving the adjusted r-squared values.
...
This analysis can be used to explain why voter turnout in Maryland varies so much across it's counties. The results show that turnout tends to be higher in places with higher income, older populations, and large portion of white residents. These patterns can help us identify where civic engagement is high and where areas there needs to be additional outreach to improve voter turnout. Also, counties that have a lower average income or younger residents may have different challenges that prevents them from participating in voting. This could be the larger story, why do these lower income areas or areas of a younger population have barriers that are preventing them from voting? 
