...
```{r}
DJIA_GDP <- read_csv("https://raw.githubusercontent.com/dwillis/jour405/refs/heads/main/data/gdp_djia.csv")
```
...
```{r}
DJIA_GDP |> summarize(correlation = cor(GDP, DJIA, method="pearson"))
```
...
DJIA_GDP |> 
  ...
  labs(
    title = "Scatterplot of DJIA and GDP",
    x = "DJIA",
    y = "GDP"
  )
```
...
Based on the data there is a strong correlation between the DJIA and GDP. The Correlation Coefficient is at 0.95, which is a very high positive correlation. Furthermore the scatterplot clearly shows the trend that there is nearly an exact linear increase for both variable if both variables increase. Essentially, one goes up, and the other one does too. Now, it is not a perfect correlation as some scatterplot points are not on the linear reggresion line, but most are.
